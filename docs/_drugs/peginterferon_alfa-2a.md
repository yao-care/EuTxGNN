...
No EU/TFDA marketing authorizations are currently on file for Peginterferon alfa-2a in this registry — market status is recorded as **Not marketed (Not Marketed)** with **0 authorizations**. This is a real regulatory data point rather than a missing-data gap, and it should be confirmed against the current EMA product register before any registration decision is made.
