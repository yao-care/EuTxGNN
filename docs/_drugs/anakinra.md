...
Anakinra is **not registered in Taiwan** (market status: Not marketed). No marketing authorizations are on file with the Taiwan FDA. Prescribers wishing to use Anakinra in Taiwan would need to apply for special import approval on a compassionate use or named-patient basis.
