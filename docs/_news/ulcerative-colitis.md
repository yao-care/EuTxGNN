---
layout: default
title: "colitis ulcerosa (ulcerative colitis) News"
parent: 📰 Health News
nav_exclude: true
description: "Health news about colitis ulcerosa (ulcerative colitis). 2 articles, 4 related drugs."
permalink: /news/ulcerative-colitis/
---

# colitis ulcerosa (ulcerative colitis) News

[← Back to News Overview]({{ '/news/' | relative_url }})

---

<p class="key-answer" data-question="What news is there about colitis ulcerosa (ulcerative colitis)?">
<strong>colitis ulcerosa (ulcerative colitis)</strong> currently has <strong>2 news articles</strong> and 4 related drugs.
</p>

<div class="key-takeaway">
This page brings together the latest health news about “colitis ulcerosa” and lists the drugs in the EuTxGNN database whose predicted indications include this disease.
</div>

<div class="related-drugs-card">
<strong>Related drug reports</strong>
<p>The predicted indications of the following drugs may be related to this disease:</p>
<ul>
<li><a href="{{ '/drugs/azathioprine/' | relative_url }}">Azathioprine</a></li>
<li><a href="{{ '/drugs/golimumab/' | relative_url }}">Golimumab</a></li>
<li><a href="{{ '/drugs/guselkumab/' | relative_url }}">Guselkumab</a></li>
<li><a href="{{ '/drugs/inebilizumab/' | relative_url }}">Inebilizumab</a></li>
</ul>
</div>

## Related News (2)

### [Oft Verdauungsprobleme? Wann gesunde Lebensmittel Entzündungen fördern könnten](https://news.google.?oc=5)

2026-09-24

Source: [t-online.de](https://news.google.?oc=5)

---

### [Cómo aplacar la colitis ulcerosa, una enfermedad que inflama el intestino - EL PAÍS](https://news.google.?oc=5)

2026-09-23

Source: [EL PAÍS](https://news.google.?oc=5)

---


<div class="disclaimer">
<strong>Disclaimer</strong>: The news on this page is collected automatically and is for research reference only; it does not constitute medical advice.
</div>
