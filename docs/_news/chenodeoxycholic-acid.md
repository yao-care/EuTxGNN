---
layout: default
title: "Chenodeoxycholic Acid 相關新聞"
parent: 健康新聞
nav_exclude: true
description: "Chenodeoxycholic Acid 的相關健康新聞報導。原適應症：Chenodeoxycholic acid is indicated for the treatme...。預測適應症 20 個。"
permalink: /news/chenodeoxycholic-acid/
---

# Chenodeoxycholic Acid 相關新聞

[← 返回新聞總覽]({{ '/news/' | relative_url }})

---

<p class="key-answer" data-question="Chenodeoxycholic Acid 有什麼相關新聞？">
<strong>Chenodeoxycholic Acid</strong> 目前有 <strong>7 則</strong>相關新聞報導，預測適應症 20 個。
</p>

<div class="key-takeaway">
本頁整合 Chenodeoxycholic Acid 的 AI 預測適應症與最新健康新聞，橘色標示的適應症表示近期有相關新聞報導。
</div>

<div class="drug-info-card">
<strong>藥物資訊</strong>
<ul>
<li><strong>原適應症</strong>：Chenodeoxycholic acid is indicated for the treatment of inborn errors of primary bile acid synthesis due to sterol 27 hydroxylase deficiency (presenting as cerebrotendinous xanthomatosis (CTX)) in...</li>
<li><strong>證據等級</strong>：L5</li>
<li><strong>預測適應症</strong>（20 個）：<ul>
<li>cholelithiasis（99.8%）</li>
<li>homozygous familial hypercholesterolemia（99.6%）</li>
<li>brain small vessel disease 1 with or without ocular anomalies（99.4%）</li>
<li>autosomal dominant familial hematuria-retinal arteriolar tortuosity-contractures syndrome（99.4%）</li>
<li>obsolete familial combined hyperlipidemia（99.3%）</li>
<li>obsolete hyperuricemia (disease)（99.3%）</li>
<li>hypouricemia, renal（98.9%）</li>
<li>diabetic nephropathy（98.8%）</li>
<li>hypoxanthine guanine phosphoribosyltransferase partial deficiency（97.8%）</li>
<li>familial isolated hypoparathyroidism due to impaired PTH secretion（97.5%）</li>
<li>Lesch-Nyhan syndrome（97.5%）</li>
<li>biotin metabolic disease（97.1%）</li>
<li>renal tubular acidosis（96.9%）</li>
<li>acromesomelic dysplasia, Campailla Martinelli type（96.7%）</li>
<li>Dahlberg-Borer-Newcomer syndrome（96.7%）</li>
<li>vitamin deficiency disorder（96.7%）</li>
<li>congestive heart failure（96.5%）</li>
<li>craniofacial conodysplasia（96.4%）</li>
<li>pseudo-von Willebrand disease（96.3%）</li>
<li>primary release disorder of platelets（96.2%）</li>
</ul></li>
</ul>
<p><a href="{{ '/drugs/chenodeoxycholic-acid/' | relative_url }}">查看完整藥物報告 →</a></p>
</div>

## 相關新聞（7 則）

### [Diabetes-Medikamente: SGLT2-Inhibitoren senken Alzheimer-Risiko um 43%](https://news.google.?oc=5)

2026-07-18 <span class="news-indication-tag">diabetes</span> <span class="news-indication-tag">AF</span>

來源：[ad-hoc-news.de](https://news.google.?oc=5)

---

### [Diabetes tipo 2 y enfermedad renal - Somos Pacientes](https://news.google.?oc=5)

2026-07-18 <span class="news-indication-tag">diabetes</span> <span class="news-indication-tag">obesidad</span>

來源：[Somos Pacientes](https://news.google.?oc=5)

---

### [Jen Gale, investigadora: “Los largos periodos sentado se relacionan con más riesgo cardiovascular, diabetes y algunos cánceres; interrumpirlos con pausas sencillas puede ayudar” - La Vanguardia](https://news.google.?oc=5)

2026-07-18 <span class="news-indication-tag">diabetes</span>

來源：[La Vanguardia](https://news.google.?oc=5)

---

### [Sobering explanation behind why people in these three countries barely get cancer](https://news.google.com/rss/articles/CBMinwFBVV95cUxNSEQ2UFFHeTZ0MVhqMzhVSUJYWjd3NUZqYlhMQU9BVElDM05vekxZSHMzV01uZktSZGFpSS1sN3BBRVlVbTUySFRLTGkxRTJPcV9qSHBZNTVmSkRhZjRJMWNxeGJqdEFGTzVwYUs2N1h2NmhTZGxfWjdyMjN2NDNvRXBHZkROdm5hWVlxbHdLUEJCRGpncWdqSEhXSFlYc0E?oc=5)

2026-07-17 <span class="news-indication-tag">diabetes</span> <span class="news-indication-tag">cancer</span>

來源：[LADbible](https://news.google.com/rss/articles/CBMinwFBVV95cUxNSEQ2UFFHeTZ0MVhqMzhVSUJYWjd3NUZqYlhMQU9BVElDM05vekxZSHMzV01uZktSZGFpSS1sN3BBRVlVbTUySFRLTGkxRTJPcV9qSHBZNTVmSkRhZjRJMWNxeGJqdEFGTzVwYUs2N1h2NmhTZGxfWjdyMjN2NDNvRXBHZkROdm5hWVlxbHdLUEJCRGpncWdqSEhXSFlYc0E?oc=5)

---

### [Pilar García Durruti, endocrina: “La diabetes tipo 2 no suele aparecer de un día para otro: en la mayoría de los casos existe una fase previa que puede prolongarse durante años sin síntomas” - La Vanguardia](https://news.google.?oc=5)

2026-07-17 <span class="news-indication-tag">diabetes</span>

來源：[La Vanguardia](https://news.google.?oc=5)

---

### [CGM für Typ-2-Diabetes: Blutzucker sinkt um einen Prozentpunkt](https://news.google.?oc=5)

2026-07-17 <span class="news-indication-tag">diabetes</span>

來源：[ad-hoc-news.de](https://news.google.?oc=5)

---

### [Nueva prueba de la importancia de una familia feliz: el trauma infantil genera enfermedades físicas de adulto - El Debate](https://news.google.?oc=5)

2026-07-17 <span class="news-indication-tag">diabetes</span>

來源：[El Debate](https://news.google.?oc=5)

---


<div class="disclaimer">
<strong>免責聲明</strong>：本頁新聞由系統自動收集，僅供研究參考，不構成醫療建議。
</div>

<style>
.indication-matched {
  background: #fff3e0;
  padding: 4px 8px;
  border-radius: 4px;
  border-left: 3px solid #ff9800;
}
.indication-tag {
  display: inline-block;
  background: #ff9800;
  color: white;
  padding: 2px 8px;
  border-radius: 12px;
  font-size: 0.8em;
  margin-left: 8px;
}
.news-indication-tag {
  display: inline-block;
  background: #ff9800;
  color: white;
  padding: 2px 10px;
  border-radius: 12px;
  font-size: 0.85em;
  margin-left: 4px;
}
.news-drug-tag {
  display: inline-block;
  background: #1565c0;
  color: white;
  padding: 2px 10px;
  border-radius: 12px;
  font-size: 0.85em;
  margin-left: 4px;
}
</style>
