---
layout: default
title: Drug Reports
nav_order: 4
has_children: true
description: "Drug repurposing predictions organized by evidence level"
permalink: /drug-reports/
---

# Drug Reports
{: .fs-9 }

Repurposing predictions organized by evidence level
{: .fs-6 .fw-300 }

---

## Evidence Level Overview

| Level | Definition | Clinical Significance |
|:-----:|------------|----------------------|
| **L1** | Multiple Phase 3 RCTs / Systematic Reviews | Strong support, consider clinical use |
| **L2** | Single RCT or multiple Phase 2 trials | Moderate support, design validation trial |
| **L3** | Observational studies / Large case series | Preliminary evidence, needs RCT |
| **L4** | Preclinical / Mechanistic / Case reports | Mechanism reasonable, lacks clinical validation |
| **L5** | AI prediction only, no clinical evidence | Research hypothesis, requires validation |

---

## Current Statistics

| Category | Count |
|----------|-------|
| Total Predictions | 32,368 |
| Unique Drugs | 638 |
| Unique Indications | 4,570 |

---

## Browse by Evidence Level

| Page | Description |
|------|-------------|
| [High Evidence (L1-L2)](/evidence-high/) | Strong clinical trial support |
| [Medium Evidence (L3-L4)](/evidence-medium/) | Preliminary evidence |
| [AI Predictions (L5)](/evidence-low/) | AI prediction only |
| [Full Drug List](/drugs/) | Browse all drugs |

---

## Disclaimer

<div style="background: #fff3cd; padding: 1rem; border-left: 4px solid #ffc107; border-radius: 4px; margin: 1rem 0;">
<strong>Caution:</strong> All predictions require clinical validation before therapeutic application. This is not medical advice.
</div>
