...
| 功能 | 狀態 | 說明 |
|------|------|------|
| EMA 藥品資料整合 | ✅ | 642 個 EMA 核准藥品 |
| TxGNN 預測 | ✅ | 32,368 個藥物-適應症預測 |
| 證據收集系統 | ✅ | ClinicalTrials.gov + PubMed（過濾已知適應症） |
| 自動化監控 | ✅ | 每日兩次檢查新證據 |
| 健康新聞監控 | ✅ | 每小時抓取歐洲新聞 |
...
| 等級 | 數量 | 說明 |
|------|------|------|
| L1 | 33 | Phase 3+ 臨床試驗證據 |
| L2 | 17 | Phase 2 臨床試驗證據 |
| L3 | 1 | 觀察性研究證據 |
| L4 | 3 | 臨床前/機制證據 |
| L5 | 588 | AI 預測（無臨床證據） |
| **總計** | **642** | |
...
| 檔案 | 用途 |
|------|------|
| `scripts/collect_evidence.py` | 批次收集證據（手動執行） |
| `scripts/update_drug_evidence.py` | 更新藥物檔案證據等級 |
...
| 頁面 | 路徑 | 說明 |
|------|------|------|
| High Evidence | `/evidence-high/` | L1 (33) + L2 (17) 藥物 |
| Medium Evidence | `/evidence-medium/` | L3 (1) + L4 (3) 藥物 |
| AI Predictions | `/evidence-low/` | L5 (588) 藥物 |
...
- [ ] 繼續收集更多證據（32,368 個配對中只處理了 505 個）
- [ ] 考慮新增 EMA 藥品核准監控（類似 TwTxGNN 的 TFDA 監控）
- [ ] 考慮新增歐洲臨床試驗註冊（EU CTR）監控
